...
RGBSpectrum::RGBSpectrum(float v = 0.0f) : CoefficientSpectrum<3>(v) {}

static RGBSpectrum RGBSpectrum::fromRGB(
    const Color rgb,
...
Vector3f RGBSpectrum::toXYZ() const {
  return RGBToXYZ(toRGB());
}

static RGBSpectrum RGBSpectrum::fromXYZ(
    const Vector3f xyz,
    SpectrumType type = SpectrumType::Reflectance) {
  RGBSpectrum r;
