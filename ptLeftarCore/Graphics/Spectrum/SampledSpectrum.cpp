...
SampledSpectrum SampledSpectrum::fromSampled(const float* lambda,
                                             const float* v,
                                             int n) {
  if (isSpectrumSamplesSorted(lambda, v, n)) {
  ...
  SampledSpectrum r;
  for (int i = 0; i < sampledSamplesCount; i++) {
    float lambda0 = lerp(float(i) / float(sampledSamplesCount),
                         sampledLambdaStart, sampledLambdaEnd);
    float lambda1 = lerp(float(i + 1) / float(sampledSamplesCount),
                         sampledLambdaStart, sampledLambdaEnd);
...
void SampledSpectrum::init() {
  for (int i = 0; i < sampledSamplesCount; i++) {
    float wl0 = lerp(float(i) / float(sampledSamplesCount), sampledLambdaStart,
                     sampledLambdaEnd);
    float wl1 = lerp(float(i + 1) / float(sampledSamplesCount),
                     sampledLambdaStart, sampledLambdaEnd);
...
float SampledSpectrum::averageSpectrumSamples(const float* lambda,
                                              const float* vals,
                                              int n,
                                              float lambdaStart,
                                              float lambdaEnd) {
...
Vector3f SampledSpectrum::toXYZ() const {
  Vector3f v;
  for (int i = 0; i < sampledSamplesCount; i++) {
    v.x += X.c[i] * c[i];
    v.y += Y.c[i] * c[i];
  ...
  float scale =
      sampledLambdaEnd -
      sampledLambdaStart / float(CIE_Y_integral * sampledSamplesCount);
...
Color SampledSpectrum::toRGB() const {
  Vector3f xyz = toXYZ();
  return XYZToRGB(xyz);
}
